...
class GermanGame:
    ...
    def __init__(self, root, data):
        ...
        button_pick_a_phrase.bind('<Button-1>', self.pick_phrase_gui)
    ...
    def pick_phrase_gui(self, *args, **kwargs):
        text_phrase = tk.Text(master=self.root, width=40, height=5, bg='light goldenrod')
        text_phrase.place(x=120, y=190)
        text_phrase.insert(tk.END, "this is my text")

        button_yes = tk.Button(master=self.root, text='Yes', font=(15), bg='green', command=yes_button)
        button_yes.place(x=220, y=290)
        #yes_button.bind('<Up>', yes_button)

        button_no = tk.Button(master=self.root, text='No', font=(15), bg='red', command=no_button)
        ...
        label_score = tk.Label(master=self.root, text='Score', bg='gold')
        label_score.place(x=840, y=30)

        text_yes_score = tk.Text(master=self.root, width=4, heigh=1, bg='green', font=('bold'))
        text_yes_score.place(x=790, y=30)
        text_yes_score.insert(tk.END, yes_score)

        text_no_score = tk.Text(master=self.root, width=4, height=1, bg='red', font=('bold'))
